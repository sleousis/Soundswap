...
/// <summary>
/// The game obscures the Vorbis data inside its music files in one of two ways: the header pages XORed with
/// one byte (mode 0x2002), or header and audio together XORed against a fixed table (mode 0x2003). The table
/// and the decoding follow VFXEditor (MIT, Copyright 2021 Michael Kaminsky); see THIRD_PARTY_NOTICES.md.
/// </summary>
internal static class ScdXor
{
    public const short HeaderXorMode = 0x2002;
    public const short TableXorMode = 0x2003;

    public static void DecodeHeader(Span<byte> header, byte key)
    {
        if (key == 0) return;
        for (var i = 0; i < header.Length; i++) header[i] ^= key;
    }

    /// <summary>Mode 0x2003: header and audio as one span; <paramref name="dataLength"/> is the entry's audio length.</summary>
    public static void DecodeTable(Span<byte> headerAndAudio, int dataLength)
    {
        var byte1 = dataLength & 0x7F;
        var byte2 = byte1 & 0x3F;
        for (var i = 0; i < headerAndAudio.Length; i++)
            headerAndAudio[i] = (byte)(Table[(byte2 + i) & 0xFF] ^ headerAndAudio[i] ^ byte1);
    }
